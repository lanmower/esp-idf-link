...
static int8_t s_lfo_depth_bipolar = 63; // Max positive depth by default (-64 to +63)
...
bool handle_filter_adjusting_pads(const bool pad_pressed_this_tick[], std::array<bool, 4>& pads_used)
{
    ...
    if (tapped_pad != -1) {
        ...
        // Action: Patch LFO to Filter if not already patched
        if (!g_filter_lfo_patched) {
             ESP_LOGD(TAG_FILTER, "Filter Adjust TAP: Patching LFO -> Filter Freq");
             g_current_synth->patchLfoToFilter(s_lfo_depth_bipolar); // Use correct state variable
             g_filter_lfo_patched = true;
             // Send initial/current shape and rate
             g_current_synth->setLfoShape(s_lfo_shape_index);
             g_current_synth->setLfoRateSync(LFO_SYNC_RATES[s_lfo_rate_index]);
             g_current_synth->setLfoSyncEnabled(true); // Always enable sync when patching via adjust
             adjustment_made = true; // Patching counts as an adjustment
        }

        // Action: Cycle LFO parameters based on which pad was tapped
        switch (tapped_pad) {
            case ARP_PAD_INDEX: // Pad 1 -> Cycle LFO Rate
                s_lfo_rate_index = (s_lfo_rate_index + 1) % NUM_LFO_SYNC_RATES;
                ESP_LOGD(TAG_FILTER, "Filter Adjust TAP: LFO Rate Sync -> Index %d (Value %d)", s_lfo_rate_index, LFO_SYNC_RATES[s_lfo_rate_index]);
                g_current_synth->setLfoRateSync(LFO_SYNC_RATES[s_lfo_rate_index]);
                adjustment_made = true;
                break;
            case 0: // Pad 0 -> Cycle LFO Shape
                 s_lfo_shape_index = (s_lfo_shape_index + 1) % NUM_LFO_SHAPES;
                 ESP_LOGD(TAG_FILTER, "Filter Adjust TAP: LFO Shape -> %d", s_lfo_shape_index);
                 g_current_synth->setLfoShape(s_lfo_shape_index);
                 adjustment_made = true;
                 break;
            default: // No other pads currently assigned specific actions
                break;
        }
