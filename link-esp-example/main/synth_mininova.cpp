...
void SynthMininova::sendNoteOff(uint8_t note) {
...
void SynthMininova::setSidechainPattern(uint8_t pattern_index) {
    ...
    // Placeholder: Use NRPN 1:80 for sidechain pattern selection
    // FIXME: Actual NRPN/CC needs verification from MiniNova MIDI spec
    send_midi_nrpn(midi_channel, 1, 80, pattern_index);
}

void SynthMininova::activateDelay() {
    send_midi_nrpn(midi_channel, FX1_SELECT_NRPN_MSB, FX1_SELECT_NRPN_LSB, FX1_EFFECT_DELAY1);
    vTaskDelay(pdMS_TO_TICKS(1)); // Small delay often needed after NRPN
}
