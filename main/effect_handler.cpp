...
void handle_sidechain_adjust_pots(int pot1_delta, int pot2_delta, bool& pot1_used, bool& pot2_used) {
    ...
    // Update the sidechain parameters that are used by handle_sidechain_active
    // Pot 1: Controls ducking depth (0 = no ducking, 127 = maximum ducking)
    _update_pot_param(s_current_sidechain_depth, pot1_delta, 0, 127, "SC Duck Depth", pot1_used);
    
    // Pot 2: Controls attack/release curve (0 = linear/slow, 127 = hard/fast)
    _update_pot_param(s_current_sidechain_sheer, pot2_delta, 0, 127, "SC Curve", pot2_used);
    
    // For MiniNova, also update the gate parameters to match the behavior
    if (g_synth_type == SYNTH_MININOVA) {
        SynthMininova* mininova = static_cast<SynthMininova*>(g_current_synth);
        
        // Map depth to wet/dry (inverted: 0 depth = 127 wet/dry, 127 depth = 0 wet/dry)
        uint8_t wetdry = 127 - s_current_sidechain_depth;
        mininova->setGateWetDry(wetdry);
        
        // Map sheer/curve to e-slew (0 = slow slew, 127 = fast slew)
        // E-Slew default is 104, so we map around that
        uint8_t eslew = 64 + (s_current_sidechain_sheer / 2); // Range: 64-127
